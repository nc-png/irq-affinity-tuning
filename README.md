...
```
sudo ./irq-affinity-auto.sh [--reduce-queues] [--dry-run] [--verbose] [--overlap-cores] [--no-adaptive-rx] [--softirq-siblings]
  ...
  --softirq-siblings pin IRQs on each core's second hyperthread instead of the
                     primary — for hosts whose application pins its workers to
                     the primaries; no effect when SMT is off
```
...
- `--softirq-siblings` moves the whole IRQ pool from each node's primary threads to their HT siblings (derived from `/sys` topology; the reserved core's sibling stays out of the pool). Only worth it when your application pins its workers to the primaries — then softirq and workers stop preempting each other through the same run-queues. Measured on a 30-slot race load: node softirq halved and node peak load dropped ~20 points, throughput unchanged. Without such worker pinning the default node-wide spread is just as good, so this stays opt-in. With SMT off it falls back to the default behavior.
...
3. `/proc/irq/*/smp_affinity_list` for every MSI-X vector of every discovered NIC, spread across physical cores of the NIC's own NUMA node (CPU0 excluded; with `--softirq-siblings`, across the cores' second hyperthreads instead)
